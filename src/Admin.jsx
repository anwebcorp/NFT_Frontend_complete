...
export default function Admin({ user, setUser }) {
    ...
    const handleManageAttendanceClick = useCallback((employee) => {
        setEmployeeForAttendance(employee); // Pass the actual employee
    ...
    // Add new handler for Dashboard view
    const handleDashboardClick = useCallback(() => {
        setEmployeeForAttendance({ id: 'dashboard', name: 'Dashboard', isDashboard: true });
        setShowAttendancePage(true);
        setShowMenu(false);
    }, []);
                ...
                >
                    ...
                    <nav className="flex flex-col p-4 space-y-2">
                        ...
                        <button
                            onClick={() => {
                                setShowMenu(false);
                                setEmployeeForAttendance({ id: 'dashboard', name: 'Dashboard' });
                                setShowAttendancePage(true);
                            }}
                            className="w-full text-left py-2 px-4 rounded-md hover:bg-neutral-100 flex items-center text-lg text-neutral-700"
                        >
